...
function installQuestions() {
    ...
    until [[ ${SERVER_WG_IPV6} =~ ^([a-f0-9]{1,4}:){3,4}: ]]; do
        read -rp "Server WireGuard IPv6: " -e -i "dd42:42:42::1" SERVER_WG_IPV6
    done
...
PrivateKey = ${SERVER_PRIV_KEY}" >"/etc/wireguard/${SERVER_WG_NIC}.conf"
    ...
    if pgrep firewalld; then
        FIREWALLD_IPV4_ADDRESS=$(printf '%s' "${SERVER_WG_IPV4}" | cut -d"." -f1-3)".0"
        FIREWALLD_IPV6_ADDRESS=$(printf '%s' "${SERVER_WG_IPV6}" | sed 's/:[^:]*$/:0/')
...
PresharedKey = ${CLIENT_PRE_SHARED_KEY}
AllowedIPs = ${CLIENT_WG_IPV4}/24,${CLIENT_WG_IPV6}/64" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
